...
select * from (select TRIM(h.cd_acao, " ") as ACAO, sum(ic_1_00_pc) as _1_00pc, sum(ic_1_50_pc) as _1_50pc, sum(ic_2_00_pc) as _2_00pc, sum(ic_2_50_pc) as _2_50pc, sum(ic_3_00_pc) as _3_00pc, avg(vr_volume) as VOLUME, sum(vr_result_1_00_pc) as sum_100_pc, sum(vr_result_1_50_pc) as sum_150_pc, sum(vr_result_2_00_pc) as sum_200_pc, avg(pc_abertura), avg(vr_fechamento), url_advfn from hist_dados h left join aluguel_acoes_advfn a on h.cd_acao = a.cd_acao
where dt_pregao in (select distinct dt_pregao 
       from hist_dados order by 1 DESC limit 25)
group by h.cd_acao having sum(ic_1_00_pc) >= 23) WHERE VOLUME > 30000000 ORDER BY 2 DESC, 3 DESC, 4 DESC, 5 DESC, 6 DESC, 7 DESC
